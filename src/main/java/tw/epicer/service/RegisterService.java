...
private Tools tools;
	...
	public RegisterService(UserDao dao, Tools tools) {
		super();
		this.dao = dao;
		this.tools = tools;
	}
	public HashMap<String,Message> checkAll(User user) {
		...
		int rdcode = road.getCode();
		if(namecode == 0 && gendercode == 0 && agecode == 0  && pswcode == 0 && phonecode ==0 && citycode == 0 && tscode == 0 && rdcode == 0) {
	...
	public boolean InsertClient(User user) {
		User result = dao.InsertCilent(user);
		if(result == null) {
			return false;
		}else {
			return true;
	...
	public Message checkAccount(User user) {
		...
		User admin = dao.checkAccount(account);
	    if(admin == null) {
	    	msg.setCode(0);
			msg.setMessage("OK");
	    }else {
