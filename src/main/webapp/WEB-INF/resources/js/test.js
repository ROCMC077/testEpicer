 ...
 * Copyright 2021 essoduke.org, Licensed MIT.
 *
 *
...
(function (root, factory) {
    ...
    TWzipcode.prototype.createElements = function () {
        ...
        Array.prototype.forEach.call(container, function (el) {
            ...
            if (el.querySelectorAll('[data-role]').length === 0) {

                const divCounty   = document.createElement('div');
                const divDistrict = document.createElement('div');
                const divZipcode  = document.createElement('div');
                el.appendChild(divCounty);
                el.appendChild(divDistrict);
                el.appendChild(divZipcode);

                const domCounty   = self.createDropdownList(divCounty, id, 'county', optCounty);
                const domDistrict = self.createDropdownList(divDistrict, id, 'district', optDistrict);
                const domZipcode  = self.createInput(divZipcode, id, 'zipcode', optZipcode);

                if (Number(opts.combine)) {
                    domZipcode.removeAttribute('required');
                    domZipcode.style.display = 'none';
                }
        ...
        //try {
            Array.prototype.forEach.call(this.container, function (el) {
                var county   = el.querySelector('[id^="county-"]'),
                    district = el.querySelector('[id^="district-"]'),
                    zipcode  = el.querySelector('[id^="zipcode-"]');
                    //console.log('county child', county.parentNode.removeChild(county));
                //county.parentNode.removeChild(county);


                if (county.parentNode) {
                    county.parentNode.removeChild(county);
                }
                if (district.parentNode) {
                    district.parentNode.removeChild(district);
                }
                if (zipcode.parentNode) {
    ...
    TWzipcode.create = function (el, options) {
         ...
         var element, instance = new TWzipcode(el, options);
         ...
         //data.set(element, 'qqq', instance);
         //element.dataset.qqq = instance;
         return instance;
    };
